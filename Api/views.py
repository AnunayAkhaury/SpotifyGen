...
class CreatePlaylist(APIView):
    ...
    def get(self,request,format = None):
        ...
        if(songName ==  False):
            return Response({'error': 'No Song is playing please try again after playing song'}, status=status.HTTP_204_NO_CONTENT)
        
        AIresponse = (SongInput(songName).content)
        AIresponse = AIresponse.split('\n')
        
        if(AddSongs(AIresponse,playlist_id, key)):
            return Response(f'CURRENT SONG{songName}  PLAYLIST CREATED WITH CUSTOM SONGS')
